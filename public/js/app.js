(function(){
	...
	app.controller('TarefasController', function($scope, $http){
		...
		$scope.adicionarTarefa = function(){
			...
			var requisicao = $http({method:"post", url:"api/tarefas",data:dadosPost}).success(function(data){
				if(data == 1){
					$scope.loadData(function(){
            $scope.texto ='';
            $scope.autor ='';
    ...
    $scope.mudarStatus = function(id,status){
      ...
      var requisicao = $http({method:"put", url:"api/tarefas/"+id,data:dadosPost}).success(function(data){
				if(data == 1){
					$scope.loadData();
				}
				else{
    ...
    $scope.excluirTarefa = function(id){
      if(confirm("Confirma a exclusão da tarefa?")){
        var requisicao = $http({method:"delete", url:"api/tarefas/"+id}).success(function(data){
  				if(data == 1){
  					$scope.loadData();
  				}
  				else{
